...
const genRouter = async (routerName, pluralName=null) => {
  const finalPluralName = pluralName ? pluralName : getPlural(routerName);
  ...
  const newFileContent = addRoutingToContent(fileContent, routerName, finalPluralName);
  fs.writeFileSync(filePath, newFileContent, 'utf8');
  console.log(`✅ Router generated: ${filePath}`);
}

const addRoutingToContent = (fileContent, routerName, pluralName=null) => {
  const newRoutes = getRouting(routerName, pluralName);
  ...
  if(insertionPointForRoutes !== -1) {
    ...
    throw new Error(`${exportLine} not found in api.js!`)
  }
}
...
const getRouting = (routerName, pluralName=null) => {
  const resourceName = capitalizeFirstLetter(routerName) + 'Controller';
  const finalPluralName = pluralName ? pluralName : getPlural(routerName.toLowerCase());
  const routePath = finalPluralName;
