...
const genSeeder = async (name, pluralName=null) => {
    const finalPluralName = pluralName ? pluralName : getPlural(name)
    ...
    let content = migrationTemplate.replace(/TableName/g, finalPluralName)
    content = content.replace(/ModelName/g, capitalizeFirstLetter(name))
    const fileName = generateSeederFileName(name)
    const filePath = `./database/seeders/${fileName}.js`
    fse.outputFile(filePath, content)
    console.log(`Genarated seeder file: ${filePath}`)
}
